<template>
    <div class="d-flex flex-column align-items-center pb-3">

        <h1 class="pb-4">Contact</h1>

        <div class="contourDiv d-flex gap-5 p-4 flex-column flex-md-row">
            <div class="d-flex flex-column align-items-center justify-content-around">
                <div class="d-flex flex-column align-items-center">
                    <p class="mb-1">Courriel:</p>
                    <p>clientTI@cegepgranby.qc.ca</p>
                </div>

                <div class="d-flex flex-column align-items-center">
                    <p class="mb-1">Adresse:</p>
                    <p class="text-center mb-1">1 rue Church Stanstead, QC J0B 3E2, Canada</p>
                    <a href="https://maps.app.goo.gl/kS5TjtZ7Q2gfChCG7" target="_blank">
                        <img src="/icons/map.png" alt="carte" height="80" />
                    </a>
                </div>
            </div>
        </div>

        <br />

        <div class="d-flex flex-column align-items-center">
            <h2>Associées</h2>

            <div class="d-flex gap-4 fs-5 flex-column flex-md-row align-items-center">
                <p>Monique Inverness</p>
                <p>Patricia DeNice</p>
                <p>Frédérique Nantes</p>
            </div>
        </div>

        <div class="mt-4 d-flex flex-column align-items-center">
            <h2>Membres de l'équipe</h2>

            <div class="d-inline-flex flex-lg-row flex-column flex-wrap gap-3 justify-content-around">
                <div class="align-items-center d-flex flex-column p-2 justify-content-between">
                    <p class="fs-5">Charles</p>
                    <p>Directeur financières</p>
                </div>

                <div class="align-items-center d-flex flex-column p-2 justify-content-between">
                    <p class="fs-5">Audrey</p>
                    <p>Directrice technique</p>
                </div>

                <div class="align-items-center d-flex flex-column p-2 justify-content-between">
                    <p class="fs-5">Albert</p>
                    <p>Directeur technique</p>
                </div>

                <div class="align-items-center d-flex flex-column p-2 justify-content-between">
                    <p class="fs-5">David</p>
                    <p class="text-center">Directeur des <br class="d-none" />opérations</p>
                </div>

                <div class="align-items-center d-flex flex-column p-2 justify-content-between">
                    <p class="fs-5">Victoria</p>
                    <p class="text-center">Responsable des <br class="d-none" />communications</p>
                </div>
            </div>
        </div>

    </div>
</template>

<script setup>
</script>

<style scoped>
</style>